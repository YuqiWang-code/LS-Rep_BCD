#!/usr/bin/env python3
"""Real-cache CUDA timing/memory/diagnostic dry run for EIR-HSD."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="EIR-HSD real-cache dry run")
    parser.add_argument("--experiment", default="R5", choices=[f"R{i}" for i in range(1, 9)])
...
def main():
    ...
    if cache.manifest.get("teacher_type") != "sam2_struct_v2":
        raise ValueError("EIR-HSD requires a sam2_struct_v2 cache")
    loader = get_loader(
    ...
    tracked_keys = (
        "code_boundary_mean", "code_local_mean", "code_geometry_mean",
        "code_stable_mean", "trust_change_mean", "trust_stable_mean",
        "coverage_change_mean", "coverage_stable_mean",
        "structural_uncertainty_mean", "fn_correction_ratio",
        "fp_correction_ratio", "encoder", "decoder_structure",
        "relation", "correction", "e_plus_mean", "e_minus_mean",
        "e_stable_mean", "e_uncertain_mean",
    )
    ...
    for step in range(1, args.steps + 1):
        ...
        details = auxiliary.get("eir_hsd", auxiliary.get("sam_hsd"))
        if details is None:
            raise RuntimeError(f"{args.experiment} did not return an auxiliary loss")
    ...
    print("EIR-HSD REAL CACHE DRY RUN PASSED")
