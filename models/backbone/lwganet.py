...
# Keep LWGANet self-contained. L0 uses drop_path=0, and this standard
# implementation also supports the non-zero L2 setting without importing timm.
...
try:
    ...
    def build_norm_layer(config, channels):
        norm_type = config.get('type', 'BN') if isinstance(config, dict) else 'BN'
        requires_grad = config.get('requires_grad', True) if isinstance(config, dict) else True
        layer = nn.SyncBatchNorm(channels) if norm_type == 'SyncBN' else nn.BatchNorm2d(channels)
    ...
    def init_weights(self, pretrained=None):
        ...
        if self.init_cfg is not None:
            if 'checkpoint' not in self.init_cfg:
                raise ValueError("init_cfg must contain a 'checkpoint' entry")
            ckpt_path = self.init_cfg['checkpoint']
        if ckpt_path is None:
            print(f'No pre-trained weights for {self.__class__.__name__}; training from scratch')
            return
        _load_pretrained(self, ckpt_path)
...
def _load_pretrained(model, checkpoint_path):
    if not checkpoint_path or not os.path.isfile(checkpoint_path):
        raise FileNotFoundError(f'LWGANet pretrained checkpoint not found: {checkpoint_path}')
    ...
    print(f'Loaded LWGANet weights from {checkpoint_path}')
    print('missing_keys: ', missing_keys)
    print('unexpected_keys: ', unexpected_keys)


def LWGANet_L0_1242_e32_k11_GELU(num_classes=2, stem_dim=32, pretrained=None,
    ...
    if pretrained is True:
        _load_pretrained(model, pretrained_path or DEFAULT_L0_PRETRAINED)
